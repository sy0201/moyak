...
export default function ResultPage() {
  ...
  const items = (location.state as { items?: DrugInfo[] })?.items ?? [];
  const drug = items[0];

  if (!drug) {
    return (
      <div className="flex flex-col bg-white min-h-screen">
        <Navigation title="검색 결과" onBack={() => navigate(-1)} />
        ...
        <div className="border border-[#E5E8EB] rounded-2xl p-4">
          <div className="flex items-start gap-4">
            {drug.image ? (
              ...
              <div className="w-14 h-14 rounded-2xl bg-[#F4F5F7] flex items-center justify-center shrink-0">
          ...
          <div className="mt-8">
            ...
            <div className="flex flex-col gap-3">
              {items.slice(1).map((item) => (
                <button
                  key={item.itemSeq}
                  onClick={() => navigate('/result', { state: { items: [item, ...items.filter(i => i.itemSeq !== item.itemSeq)] } })}
